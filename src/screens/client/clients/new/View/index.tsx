...
import { PATH_ADMIN, PATH_COMPANY } from 'routes/paths';
...
const View: React.FC<RegisterProps> = ({ dataCnpj, loading, dataAutocomplete, typePage }) => {
  ...
  const NewCompanySchema = Yup.object().shape({
    ...
    responsible_phone: Yup.string().required('Telefone do Responsável é obrigatório')
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      ...
      //     responsavel_nome: values.responsible_name,
      //     responsavel_telefone: values.responsible_phone
      //   };
      //   try {
      ...
      //         variant: 'success'
      //       });
      //       navigate(PATH_COMPANY.general.management.clients);
      //     } else {
  ...
  useEffect(() => {
    ...
    if (dataAutocomplete && dataAutocomplete.estabelecimento) {
      ...
      formik.setFieldValue('responsible_name', dataAutocomplete?.responsavel_nome);
      formik.setFieldValue('responsible_phone', dataAutocomplete?.responsavel_telefone);
      setValidRegister(dataAutocomplete?.ativo);
                  ...
                  <Stack spacing={3}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 2, sm: 2 }}>
                      ...
                      <InputMask
                        ...
                        disabled={false}
                        maskChar=""
                        {...getFieldProps('responsible_phone')}
                        error={Boolean(touched.responsible_phone && errors.responsible_phone)}
                      >
                        {(inputprops: any) => (
                          <TextField fullWidth label="Telefone" {...inputprops} />
                        )}
                      </InputMask>
                    </Stack>
                    {/* <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 2, sm: 2 }}>
                    ...
                    {typePage === 'Edit' ? (
                      ...
                      /> */}
                          <FormControlLabel
                            ...
                            onChange={() => {
                              formik.setFieldValue('active', !validRegister);
