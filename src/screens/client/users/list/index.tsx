...
import { fetchUsers } from 'services/store/actions/client/user.action';
import View from './View';

const ClientListUsers: React.FC = () => {
  ...
  const [users, usersLoading] = useAppSelector((state) => [
    state.UsersClient.users,
    state.UsersClient.usersLoading
  ]);
  ...
  const handleDelete = async (userId: number) => {
    const endpoint = `${process.env.REACT_APP_BASE_URL}${endpoints.client.user}/${userId}`;
  ...
  const handleToggleStatus = async (userId: string, status: boolean) => {
    ...
    const endpoint = `${process.env.REACT_APP_BASE_URL}${endpoints.client.user}/${userId}/change-status`;
