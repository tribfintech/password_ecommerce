...
import { fetchGroups } from 'services/store/actions/client/user.action';
import View from './View';

const ClientGroupsList: React.FC = () => {
  const dispatch = useAppDispatch();

  const { themeStretch } = useSettings();
  const [selected, setSelected] = useState<string[]>([]);
  const [list, setList] = useState<ClientGroupListResponseProps[]>([]);
  ...
  const [groups, loading] = useAppSelector((state) => [
    state.UsersClient.groups,
    state.UsersClient.groupsLoading
  ]);
  ...
  const handleDelete = async (id: number) => {
    const endpoint = `${process.env.REACT_APP_BASE_URL}${endpoints.client.groups}/${id}`;
